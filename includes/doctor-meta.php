...
add_action( 'manage_doctor_posts_custom_column', function( $column, $post_id ) {
    ...
    $raw_ids = get_post_meta( $post_id, 'clinic_id', true );
    if ( ! is_array( $raw_ids ) ) {
        if ( strpos( $raw_ids, '|' ) !== false ) {
...
function cpt360_render_doctor_clinic_metabox( $post ) {
    ...
    // 4a) Raw meta value (could be array or serialized string)
    $raw = get_post_meta( $post->ID, 'clinic_id', true );

    // 4b) Normalize into an array of ints
    if ( ! is_array( $raw ) ) {
        if ( strpos( $raw, '|' ) !== false ) {
            $selected = array_map( 'intval', explode( '|', $raw ) );
    ...
    // 4d) Render the multi-select
    echo '<select name="clinic_id[]" multiple style="width:100%; min-height:8em;">';
...
add_action( 'save_post_doctor', function( $post_id ) {
    ...
    // c) Sanitize & store
    $new = [];
    if ( ! empty( $_POST['clinic_id'] ) && is_array( $_POST['clinic_id'] ) ) {
        $new = array_map( 'intval', wp_unslash( $_POST['clinic_id'] ) );
    }
    if ( $new ) {
        update_post_meta( $post_id, 'clinic_id', $new );
    } else {
        delete_post_meta( $post_id, 'clinic_id' );
    }
}, 10, 2 );

/* 6) Doctor Details metabox (name, title, bio, photo) */
add_action( 'add_meta_boxes', function() {
    add_meta_box(
        'doctor_details',
    ...
    <div id="doctor-details-metabox">
      <p>
        ...
        <input type="text" id="doctor_name" name="doctor_name"
               value="<?php echo esc_attr( $name ); ?>" style="width:100%;" />
      </p>
      <p>
      ...
      <p>
        ...
        <textarea id="doctor_bio" name="doctor_bio"
                  rows="5" style="width:100%;"><?php echo esc_textarea( $bio ); ?></textarea>
      </p>
      <p>
        ...
        <input type="hidden" id="doctor_photo_field"
               name="doctor_photo_field"
               value="<?php echo esc_attr( $photo_id ); ?>" />
        <button type="button" class="button" id="doctor_photo_button">
          <?php echo $photo_id
                 ? esc_html__( 'Change Photo', 'cpt360' )
                 : esc_html__( 'Select Photo', 'cpt360' ); ?>
        </button>
        <button type="button" class="button" id="doctor_photo_remove"
...
add_action( 'save_post_doctor', function( $post_id ) {
    ...
    if ( isset( $_POST['doctor_bio'] ) ) {
        update_post_meta( $post_id, 'doctor_bio', sanitize_textarea_field( $_POST['doctor_bio'] ) );
    }
